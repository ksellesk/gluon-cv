...
def get_dataset(dataset, args):
    if dataset.lower() == 'voc':
        ...
        train_dataset = gdata.PersonDetection(root=COCO_ROOT_DIR, splits=('person_train2014', 'person_train2017'), use_crowd=False)
...
if __name__ == '__main__':
    ...
    if num_sync_bn_devices > 1:
        net = get_model(net_name, classes=COCO_CLASSES, transfer=args.dataset, num_sync_bn_devices=num_sync_bn_devices)
        async_net = get_model(net_name, classes=COCO_CLASSES, transfer=args.dataset)  # used by cpu worker
    else:
